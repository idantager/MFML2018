...
namespace DataSetsSparsity
{
    class DB
    {
        ...
        public double[][] getcleanDb(string filename, long lineCount = 0)
        {
            ...
            double[][] training_dt = new double[lineCount][];
            training_dt[0] = new double[values.Count()];
            for (int j = 0; j < values.Count(); j++)
                training_dt[0][j] = double.Parse(values[j]);
            ...
            while (!reader.EndOfStream)
            {
                line = reader.ReadLine();
                values = line.Split(seperator, StringSplitOptions.RemoveEmptyEntries);
                training_dt[counter] = new double[values.Count()];
                for(int j =0; j < values.Count();j++)
                {
                    double val = double.Parse(values[j]);
                    training_dt[counter][j] = val;
                    minsD[j] = (val < minsD[j]) ? val : minsD[j];
                    maxsD[j] = (val > maxsD[j]) ? val : maxsD[j];
                }
        ...
        public double[][] getboundingBox(double[][] dt)
        {
            ...
            for (int i = 0; i < Ncol ; i++)
            {
                if (BB[0][i] != BB[1][i])
                {
                    ...
                    BB[0][i] -= 0.00000000000001 * (tmp_a - tmp_b);
                    BB[1][i] += 0.00000000000001 * (tmp_a - tmp_b);                
                }
            }
        ...
        public static bool IsPntInsideBox(int[][] BoxOfIndeces, double[] pnt, int dim)
        {
            for (int i = 0; i < dim; i++)
            {
                if (pnt[i] == 55555.66666)//NA ELEMENT
                    continue;
                if (pnt[i] < wf.Program.MainGrid[i][BoxOfIndeces[0][i]] || pnt[i] > wf.Program.MainGrid[i][BoxOfIndeces[1][i]])
                    return false;
            }
            return true;
        }
        ...
        public static void ProjectPntInsideBox(int[][] BoxOfIndeces, ref double[] pnt)
        {
            ...
            {
                if (pnt[i] < wf.Program.MainGrid[i][BoxOfIndeces[0][i]])
                    pnt[i] = wf.Program.MainGrid[i][BoxOfIndeces[0][i]];
                if (pnt[i] > wf.Program.MainGrid[i][BoxOfIndeces[1][i]])
                    pnt[i] = wf.Program.MainGrid[i][BoxOfIndeces[1][i]];
            }
        }
    }
}
